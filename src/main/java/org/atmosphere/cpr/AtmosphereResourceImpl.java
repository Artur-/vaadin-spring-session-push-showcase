...
public class AtmosphereResourceImpl implements AtmosphereResource {
    ...
    private transient AtmosphereResponse response;
    private final Action action = new Action();
    protected transient List<Broadcaster> broadcasters = new CopyOnWriteArrayList<Broadcaster>();
    protected transient Broadcaster broadcaster;
    private transient AtmosphereConfig config;
    protected transient AsyncSupport asyncSupport;
    ...
    private transient AtmosphereHandler atmosphereHandler;
    private String uuid;
    ...
    private void readObject(ObjectInputStream stream)
        ...
        VaadinServletService service = (VaadinServletService)UI.getCurrent().getSession().getService();
        //TODO: if deserialization would work I should use req.getServletContext()
        AtmosphereFramework atmosphere = (AtmosphereFramework)WebApplicationContextUtils.getWebApplicationContext(service.getServlet().getServletContext()).getBean("atmosphereFramework");
        config = atmosphere.getAtmosphereConfig();
        asyncSupport = atmosphere.getAsyncSupport();
        atmosphereHandler = atmosphere.getAtmosphereHandlers().values().iterator().next().atmosphereHandler;

        this.broadcasters = new CopyOnWriteArrayList<Broadcaster>();
        Broadcaster baseBroadCaster = atmosphere.getBroadcasterFactory().lookupAll().iterator().next();
        baseBroadCaster.addAtmosphereResource(this);
        this.setBroadcaster(baseBroadCaster);
    }
